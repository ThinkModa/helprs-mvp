...
-- For now, just add the new fields and we'll handle enum updates separately
-- The existing enum should work for now
...
SET assigned_worker_id = (SELECT id FROM workers LIMIT 1),
    ...
    status = 'scheduled'
WHERE status = 'open' 
AND id IN (SELECT id FROM jobs WHERE status = 'open' LIMIT 2);
